...
type Building = {
  ...
  address?: string;
  unitCount?: number;
  rooms?: Array<{ houseNumber?: string; isVacant?: boolean }>;
  updatedAt?: string;
};
...
type StartupPayload = {
  buildings?: Building[];
  paymentAccess?: PaymentAccessRecord[];
  residentDirectory?: ResidentDirectoryRow[];
  rentStatus?: ResidentDirectoryRow[];
  ownerStaff?: unknown[];
};
...
type RentSetupRow = {
  ...
  rentSetupSource?: string;
  resolvedMonthlyRentKsh?: number | null;
  resolvedDueDay?: number | null;
  resolvedGraceDays?: number | null;
  roomDefaultMonthlyRentKsh?: number | null;
  roomDefaultDueDay?: number | null;
  roomDefaultGraceDays?: number | null;
  roomDefaultActive?: boolean;
  depositKsh?: number | null;
  balanceKsh?: number | null;
  currentMonthPaidKsh?: number | null;
  arrearsKsh?: number | null;
};
...
type RentSetupSheet = {
  ...
  buildingName: string;
  rentEnabled?: boolean;
  buildingDefaultMonthlyRentKsh?: number | null;
  buildingDefaultDepositKsh?: number | null;
  buildingDefaultDueDay?: number | null;
  buildingDefaultGraceDays?: number | null;
  chargeStartDate?: string | null;
  updatedCount?: number;
  rows?: RentSetupRow[];
};

type RentSetupPayload = {
  buildingDefaultMonthlyRentKsh: number | null;
  buildingDefaultDepositKsh: number | null;
  buildingDefaultDueDay: number | null;
  buildingDefaultGraceDays: number;
  chargeStartDate: string | null;
  note?: string;
  rows: Array<{
    houseNumber: string;
...
type RoomRow = {
  ...
  residentName: string;
  phone: string;
  status: "occupied" | "vacant" | "pending" | "unknown";
  rentKsh: number;
  depositKsh: number;
  utilitiesKsh: number;
  balanceKsh: number;
...
type AppState = {
  ...
  selectedStatus: "all" | RoomRow["status"];
  search: string;
  loading: boolean;
  selectedPaymentRoom: RoomRow | null;
  rentPaymentStatus: string;
  rentPaymentSaving: boolean;
  rentSetupBuildingId: string;
  rentSetupSheet: RentSetupSheet | null;
  rentSetupLoading: boolean;
  rentSetupSaving: boolean;
  rentSetupStatus: string;
};

const app = document.querySelector<HTMLDivElement>("#management-app");
...
function optionalInput(value: unknown): string {
...
async function apiEnvelope<T>(path: string, init: RequestInit = {}): Promise<ApiEnvelope<T>> {
  ...
  return { data: payload.data as T, role: payload.role, mode: payload.mode };
}

function normalizeStatus(row: ResidentDirectoryRow): RoomRow["status"] {
  const raw = text(row.status || row.occupancyStatus).toLowerCase();
  if (raw.includes("occupied") || raw.includes("active")) return "occupied";
  if (raw.includes("vacant")) return "vacant";
  if (raw.includes("pending")) return "pending";
  return text(row.residentName || row.name).toLowerCase() === "vacant" ? "vacant" : "unknown";
}

function roomFromResident(row: ResidentDirectoryRow, fallbackBuilding?: Building): RoomRow {
...
function buildRooms(payload: StartupPayload): RoomRow[] {
...
function filteredRooms(): RoomRow[] {
  const query = state.search.toLowerCase();
  return state.rooms.filter((room) => {
    const buildingMatches =
      state.selectedBuildingId === "all" || room.buildingId === state.selectedBuildingId;
...
function roomTotals(rows: RoomRow[]) {
  return rows.reduce(
    (summary, room) => {
      summary.rooms += 1;
...
function rentEnabledBuildings(): Building[] {
  const accessByBuilding = new Map(
    state.paymentAccess.map((item) => [text(item.buildingId), item.rentEnabled])
  );
  return state.buildings.filter((building) => accessByBuilding.get(building.id) !== false);
}

function getRentSetupBuildingId(): string {
...
function roomUrl(room: RoomRow): string {
  return `/landlord/rooms/${encodeURIComponent(room.buildingId)}/${encodeURIComponent(
    room.houseNumber
  )}`;
}
...
function renderOperationsHeader(): string {
...
function renderRoomsView(): string {
  const rows = filteredRooms();
  const summary = roomTotals(rows);
  const buildingOptions = state.buildings
    .map(
      (building) =>
        `<option value="${escapeAttr(building.id)}"${building.id === state.selectedBuildingId ? " selected" : ""}>${escapeHtml(
  ...
  return `
    <section class="management-toolbar">
      ...
      <div class="management-controls">
        ...
        <label>
          Search
          <input id="room-search" type="search" value="${escapeAttr(
            state.search
          )}" placeholder="House, resident, phone" />
        </label>
...
function renderRentPaymentPanel(): string {
...
function renderRentSetupView(): string {
  const selectedBuildingId = getRentSetupBuildingId();
  const buildingOptions = rentEnabledBuildings()
    .map(
      (building) =>
        `<option value="${escapeAttr(building.id)}"${building.id === selectedBuildingId ? " selected" : ""}>${escapeHtml(
          buildingName(building)
        )}</option>`
    )
    .join("");
  ...
  const rows = [...(sheet?.rows || [])].sort((left, right) =>
    text(left.houseNumber).localeCompare(text(right.houseNumber), undefined, { numeric: true })
  );
...
function rentDefaultField(label: string, id: string, value: unknown, placeholder: string): string {
  return `
    <label>
      ${escapeHtml(label)}
      <input id="${escapeAttr(id)}" type="number" min="0" step="1" value="${escapeAttr(
        optionalInput(value)
...
function renderRentSetupRow(row: RentSetupRow): string {
  ...
  return `
    <tr data-rent-row="true" data-house-number="${escapeAttr(houseNumber)}" data-resolved-rent="${escapeAttr(
      resolvedRent
    )}" data-original-paid="${escapeAttr(currentMonthPaid)}">
...
function statusOption(value: AppState["selectedStatus"], label: string): string {
  return `<option value="${value}"${state.selectedStatus === value ? " selected" : ""}>${label}</option>`;
}

function metric(label: string, value: string | number): string {
  return `<div class="management-metric"><span>${escapeHtml(label)}</span><strong>${escapeHtml(
    value
  )}</strong></div>`;
}

function renderRoomRow(room: RoomRow): string {
  return `
    <tr>
...
function rentSetupSourceLabel(source: unknown): string {
  const value = text(source);
  if (value === "room_default") return "Room Default";
  if (value === "building_default") return "Building Default";
  if (value === "agreement_legacy") return "Tenant Record";
  if (value === "room_disabled") return "No Charge";
  return "Unset";
}

function formatRentTiming(row: RentSetupRow): string {
...
function bindRenderedControls(): void {
  ...
  document.querySelector<HTMLSelectElement>("#building-filter")?.addEventListener("change", (event) => {
    state.selectedBuildingId = (event.currentTarget as HTMLSelectElement).value;
    render();
  });

  document.querySelector<HTMLSelectElement>("#status-filter")?.addEventListener("change", (event) => {
    state.selectedStatus = (event.currentTarget as HTMLSelectElement).value as AppState["selectedStatus"];
    render();
  });

  document.querySelector<HTMLInputElement>("#room-search")?.addEventListener("input", (event) => {
    const target = event.currentTarget as HTMLInputElement;
    const cursor = target.selectionStart;
    state.search = target.value;
    render();

    const nextSearch = document.querySelector<HTMLInputElement>("#room-search");
    if (nextSearch) {
      nextSearch.focus();
      if (cursor !== null) {
  ...
  document.querySelector<HTMLButtonElement>("#refresh-management")?.addEventListener("click", () => {
    void load();
  });

  document.querySelectorAll<HTMLButtonElement>('[data-action="record-rent"]').forEach((button) => {
    button.addEventListener("click", () => {
      const buildingId = text(button.dataset.buildingId);
      const houseNumber = text(button.dataset.houseNumber);
      state.selectedPaymentRoom = state.rooms.find(
        (room) => room.buildingId === buildingId && room.houseNumber === houseNumber
      ) ?? null;
      ...
      render();
      document.querySelector<HTMLInputElement>("#rent-payment-amount")?.focus();
    });
  });

  document.querySelector<HTMLFormElement>("#rent-payment-form")?.addEventListener("submit", (event) => {
    event.preventDefault();
    void submitRentPayment();
  });

  document.querySelector<HTMLSelectElement>("#rent-building-filter")?.addEventListener("change", (event) => {
    state.rentSetupBuildingId = (event.currentTarget as HTMLSelectElement).value;
    state.rentSetupSheet = null;
    void loadRentSetup();
  });

  document.querySelector<HTMLButtonElement>("#load-rent-setup")?.addEventListener("click", () => {
    void loadRentSetup();
  });

  document.querySelector<HTMLButtonElement>("#save-rent-setup")?.addEventListener("click", () => {
    void saveRentSetup();
  });
}
...
async function load(): Promise<void> {
  ...
  try {
    const response = await apiEnvelope<StartupPayload>("/api/landlord/startup?mode=quick");
    ...
    state.rooms = buildRooms(payload).sort((left, right) =>
      `${left.buildingName}-${left.houseNumber}`.localeCompare(`${right.buildingName}-${right.houseNumber}`, undefined, {
        numeric: true
      })
    );
    ...
    if (app) {
      app.innerHTML = `<div class="management-error"><strong>Could not load management data.</strong><p>${escapeHtml(
        error instanceof Error ? error.message : "Unknown error"
      )}</p><a href="/landlord">Open classic workspace</a></div>`;
    }
    return;
  } finally {
    state.loading = false;
...
async function submitRentPayment(): Promise<void> {
  const room = state.selectedPaymentRoom;
  if (!room) return;

  const amountKsh = optionalNumber(document.querySelector<HTMLInputElement>("#rent-payment-amount")?.value ?? "");
  const provider = text(document.querySelector<HTMLSelectElement>("#rent-payment-provider")?.value, "cash");
  const providerReference = text(document.querySelector<HTMLInputElement>("#rent-payment-reference")?.value);
  const billingMonth = text(document.querySelector<HTMLInputElement>("#rent-payment-month")?.value) || undefined;
  const paidAt = toIsoFromDateTimeLocal(document.querySelector<HTMLInputElement>("#rent-payment-paid-at")?.value ?? "");
  ...
  try {
    const response = await apiEnvelope<RentPaymentResult>(
      `/api/landlord/rent/${encodeURIComponent(room.houseNumber)}/payments`,
      {
        method: "POST",
        body: JSON.stringify({
    ...
    const refreshedRoom = state.rooms.find(
      (item) => item.buildingId === room.buildingId && item.houseNumber === room.houseNumber
    );
...
async function loadRentSetup(): Promise<void> {
  ...
  try {
    const response = await apiEnvelope<RentSetupSheet>(
...
async function saveRentSetup(): Promise<void> {
  const buildingId = state.rentSetupSheet?.buildingId || getRentSetupBuildingId();
  if (!buildingId || !state.rentSetupSheet) return;

  let payload: RentSetupPayload;
  try {
...
async function refreshRoomsQuietly(): Promise<void> {
  try {
    const response = await apiEnvelope<StartupPayload>("/api/landlord/startup?mode=quick");
    const payload = response.data;
...
function buildRentSetupPayload(): RentSetupPayload {
  ...
  const chargeStartDate = text(document.querySelector<HTMLInputElement>("#charge-start-date")?.value) || null;
  const note = text(document.querySelector<HTMLInputElement>("#rent-setup-note")?.value);
  ...
  const rows = [...document.querySelectorAll<HTMLTableRowElement>('tr[data-rent-row="true"]')].map((row) => {
    ...
    const active = row.querySelector<HTMLInputElement>('input[data-field="active"]')?.checked ?? true;

    if (!houseNumber) throw new Error("A rent setup row is missing its house number.");
    ...
    const resolvedRent = Math.max(
      0,
      monthlyRentKsh ?? buildingDefaultMonthlyRentKsh ?? numberValue(row.dataset.resolvedRent)
    );
...
function readOptionalNumber(selector: string, label: string): number | null {
...
function readRowOptionalNumber(row: HTMLTableRowElement, field: string, label: string): number | null {
  const value = row.querySelector<HTMLInputElement>(`input[data-field="${field}"]`)?.value ?? "";
  const parsed = optionalNumber(value);
  if (parsed != null && parsed < 0) {
    throw new Error(`${label} cannot be negative.`);
  }
